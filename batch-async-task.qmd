...
函数采用 pbapply 显示进度，`batch_size` 设定批次大小，`fun` 接受命令（如果这个命令是一个 memorise 函数则直接使用，如果不是则将其自动缓存），缓存默认使用 `cache_filesystem`，缓存文件地址为 `cache_dir` 目录下的 `id` 文件夹，必需 id 由用户提供。

```{r}
batch_async_task <- function(df, fun, id, wait_time = 1, batch_size = 10, cache_dir = "cache") {
  # 导入必要的包
  require(future)
  ...
  # 检查并创建缓存目录
  dir_path <- file.path(cache_dir, id)
  ...
  if (inherits(fun, "memoised")) {
    warning("Fun is already a memoised function. It will be used directly. `cache_dir` will be ignored.")
  } else {
    # 如果fun不是memoise函数，则将其缓存
    fun <- memoise(fun, cache = cache_filesystem(dir_path))
  }
...
# 定义 cached_process_function
cached_process_function <- memoise(process_function, cache = cache_filesystem("cache"))
```

```{r}
# 运行函数
result1 <- batch_async_task(df, process_function, id = "test")

result2 <- batch_async_task(df, cached_process_function, id = "test")
