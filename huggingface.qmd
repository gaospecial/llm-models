...
Hugging Face 是一个专注于人工智能（AI）和机器学习（ML）的平台，特别以自然语言处理（NLP）工具和模型而闻名。它提供了一个丰富的开源生态系统，供研究人员、开发者和数据科学家使用。其中，最著名的工具之一是 **Transformers** 库，这个库包含了多种预训练的模型（如BERT、GPT、T5等），这些模型可以用于各种NLP任务，如文本生成、翻译、问答、情感分析等。
...
总结来说，虽然模型可以从 Hugging Face Hub 下载，但任务执行是在本地进行的。
...
不过，调用 `pipeline("tranlation_en_to_zh")` 却会出错。这是因为：虽然 "translation_en_to_zh" 是一个有效的任务标识符，但它并不是直接指定模型的名称。有些错误可能是由于没有找到合适的预训练模型。你可以显式指定模型名称来避免这种情况：

::: {.callout-note}
这里还需要安装一个确实的模块：`SentencePiece`。
...
```{python}
translator = pipeline("translation_en_to_zh", model="Helsinki-NLP/opus-mt-en-zh", device=device)
translator("This is a introduction to Huggingface.")
```

结果一般吧！

翻译任务需要用到的功能包括分词，模型的选择等。下面的代码进行了详细的设置。
...
translation = pipeline("translation_en_to_zh", model=model, tokenizer=tokenizer, device=device)
